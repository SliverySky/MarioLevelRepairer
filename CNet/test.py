...
def test_legal(name, change_n):
    ...
    for i in range(len(cnt)):
        print('Type', i, " Elimated=", cnt[i], "/", total[i], ' Rate=%.1f%%' % (100 * cnt[i] / total[i]) if total[i] else 0)
        cnt_sum += cnt[i]
        total_sum += total[i]
...
if __name__ == '__main__':
    print("Experiment 1. Results of identifying legal tiles. The number of legal tiles which are wrongly eliminated by CNet.")
    for i in range(4):
        print("True surrounding info" if i == 0 else "Fake surrounding info with "+str(i)+"randomly"+"changed element")
        test_legal('legal_rule', i)
    print("Experiment 2. Results of detecting illegal tiles. The number of illegal tiles which are detected correctly by CNet.")
    for i in range(4):
        print("True surrounding info" if i == 0 else "Fake surrounding info with "+str(i)+"randomly"+"changed element")
