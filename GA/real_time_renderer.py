    ...
    def draw_help(self):
        if self.status == 1:
            text_surface = self.font.render(
                'press left to see previous, right to watch next, space to continue',
                True, (0, 0, 0)
            )
        else:
            text_surface = self.font.render(
        ...
        self.surface.blit(text_surface, [200, 40])

    def draw_level(self):
        img = image.load(Renderer.path + 'iteration%d.png' % self.iter)
