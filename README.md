...
3. **Generate levels to repair**: 
   Run ```python LevelGenerator/RandomDestroyed/generate.py``` to generate random destroyed levels for the further test. As we mentioned in the paper, GAN will generate broken pipes sometimes. We prepared a trained GAN model in "*LevelGenerator/GAN/generator.pth*". You can run ```python LevelGenerator/GAN/generate_level.py```  to generate some levels by GAN to see how many defective level it will generate.  
